...
export default {
  ...
  computed: {
    ...
    isEvolutionInbox() {
      return this.currentInbox.channel_type === 'Channel::Evolution';
    },
    ...
    message() {
      if (this.isEvolutionInbox) {
        return `${this.$t('INBOX_MGMT.FINISH.MESSAGE')}. ${this.$t(
          'INBOX_MGMT.ADD.EVOLUTION.FINISH_MESSAGE'
        )}`;
      }
  ...
  >
    ...
    >
      <div class="w-full text-center">
        ...
        <div v-if="isEvolutionInbox" class="w-[50%] max-w-[50%] ml-[25%]">
          <p class="mt-8 font-medium text-slate-700 dark:text-slate-200">
            {{ $t('INBOX_MGMT.ADD.EVOLUTION.FINISH_QR_INSTRUCTION') }}
          </p>
          <woot-code
            lang="markdown"
            :script="currentInbox.provider_config.qrcode"
          />
        </div>
